...
class CustomTrainer(train.BaseTrainer):
    def __init__(self, args):
        ...
        if self.use_flow_matching:
            ...
            self.duogesture_base = GestureLSMBaseMotion(self.args).to(self.device)
            ...
            self.duogesture_base = getattr(base_model_module, "duogesture_base")(self.args).to(self.device)
            try:
        ...
        self.vq_model_face = getattr(rvq_model_module, "RVQVAE")(self.args).to(self.device)
        ...
        self.vq_model_upper = getattr(rvq_model_module, "RVQVAE")(self.args).to(self.device)
        ...
        self.vq_model_hands = getattr(rvq_model_module, "RVQVAE")(self.args).to(self.device)
        ...
        self.vq_model_lower = getattr(rvq_model_module, "RVQVAE")(self.args).to(self.device)
        ...
        self.args.vae_layer = 4
        self.global_motion = getattr(vq_model_module, "VAEConvZero")(self.args).to(self.device)
        ...
        self.cls_loss = nn.NLLLoss(reduction='none').to(self.device)
        self.reclatent_loss = nn.MSELoss(reduction='none').to(self.device)
        self.vel_loss = torch.nn.L1Loss(reduction='mean').to(self.device)
        self.rec_loss = get_loss_func("GeodesicLoss").to(self.device) 
        self.log_softmax = nn.LogSoftmax(dim=2).to(self.device)
        self.reclatent_loss_test = nn.MSELoss().to(self.device)
        ...
        if gate_class_weight != 1.0:
            w = torch.tensor([1.0, float(gate_class_weight)]).to(self.device)
            self.gate_fuc = nn.CrossEntropyLoss(weight=w).to(self.device)
        else:
            self.gate_fuc = nn.CrossEntropyLoss().to(self.device)
    ...
    def inverse_selection_tensor(self, filtered_t, selection_array, n):
        ...
        original_shape_t = torch.zeros((n, 165)).to(self.device)
    ...
    def _g_training(self, loaded_data, use_adv, mode="train", epoch=0):
        ...
        mask_val = torch.ones(bs, n, self.args.pose_dims+3+4).float().to(self.device) # same shape as latent_all
        mask_val[:, :self.args.pre_frames, :] = 0.0  # first frames are seed poses (paper Sec. 5.2.1)
        latent_val = self.duogesture_base.forward_latent(loaded_data['beat'].to(self.device), loaded_data['in_word'].to(self.device), mask=mask_val, in_id = loaded_data['tar_id'].to(self.device), in_motion = loaded_data['latent_all'].to(self.device), use_attentions = True,hubert=loaded_data['hubert'].to(self.device))
        ...
        sem_mean = loaded_data['sem_mean'].to(self.device)
        sem_label = (sem_mean>0).long().to(self.device)
        ...
        #     # ------ masked gesture moderling------ #
            ...
            latent_self = self.duogesture_base.forward_latent(loaded_data['beat'].to(self.device), loaded_data['in_word'].to(self.device), mask=mask, in_id = loaded_data['tar_id'].to(self.device), in_motion = loaded_data['latent_all'].to(self.device), use_attentions = True,use_word=False,hubert=loaded_data['hubert'].to(self.device))
            ...
            latent_word = self.duogesture_base.forward_latent(loaded_data['beat'].to(self.device), loaded_data['in_word'].to(self.device), mask=mask_val, in_id = loaded_data['tar_id'].to(self.device), in_motion = loaded_data['latent_all'].to(self.device), use_word=True, use_attentions = True,hubert=loaded_data['hubert'].to(self.device))
    ...
    def _g_test(self, loaded_data):
        mode = 'test'
        bs, n, j = loaded_data["tar_pose"].shape[0], loaded_data["tar_pose"].shape[1], self.joints 
        tar_pose = loaded_data["tar_pose"].to(self.device)
        tar_beta = loaded_data["tar_beta"].to(self.device)
        in_word = loaded_data["in_word"].to(self.device)
        tar_exps = loaded_data["tar_exps"].to(self.device)
        tar_contact = loaded_data["tar_contact"].to(self.device)
        tar_trans = loaded_data["tar_trans"]
        hubert = loaded_data["hubert"].to(self.device)
        beat = loaded_data["beat"].to(self.device)
        emo_clip_text = loaded_data["emo_clip_text"].to(self.device)
        feat_clip_text = loaded_data["feat_clip_text"].to(self.device)
        remain = n%8
        ...
        for i in range(0, roundt):
            ...
            mask_val = torch.ones(bs, self.args.pose_length, self.args.pose_dims+3+4).float().to(self.device)
    ...
    def inference(self, audio_path, out_name=None):
        ...
        tar_exps = loaded_data["tar_exps"].to(self.device)
        tar_contact = loaded_data["tar_contact"].to(self.device)
        tar_trans = loaded_data["tar_trans"].to(self.device)
