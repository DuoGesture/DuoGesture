...
class BaseTrainer(object):
    def __init__(self, args):
        ...
        if args.ddp:
            # In DDP mode, the rank corresponds to the GPU id.
            self.device = torch.device(f"cuda:{self.rank}")
        else:
            # Single machine multi-card / single card
        ...
        if args.ddp:
            self.model = getattr(model_module, args.g_name)(args).to(self.device)
            ...
            _model_inst = getattr(model_module, args.g_name)(args)
            if torch.cuda.is_available():
                self.model = torch.nn.DataParallel(_model_inst, args.gpus).to(self.device)
            else:
                self.model = torch.nn.DataParallel(_model_inst)
        ...
        if args.d_name is not None:
            if args.ddp:
                ...
                _d_inst = getattr(model_module, args.d_name)(args)
                if torch.cuda.is_available():
                    self.d_model = torch.nn.DataParallel(_d_inst, args.gpus).to(self.device)
                else:
                    self.d_model = torch.nn.DataParallel(_d_inst)
            if self.rank == 0:
    ...
    def inverse_selection_tensor_6d(self, filtered_t, selection_array, n):
        ...
        selected_indices = torch.where(selection_array == 1)[0]
        if len(filtered_t.shape) == 2:
            original_shape_t = torch.zeros((n, 330)).to(self.device)
            for i in range(n):
                original_shape_t[i, selected_indices] = filtered_t[i]
        elif len(filtered_t.shape) == 3:
            bs, n, _ = filtered_t.shape
...
def main_worker(rank, world_size, args):
    ...
    # MUST come before init_process_group so that .to(self.device) calls inside __init__
    ...
    if n_visible > 0:
        ...
        print(f"[rank {rank}] No CUDA devices found — running on CPU", flush=True)
    ...
    dist.init_process_group(backend=_backend, rank=rank, world_size=world_size,
